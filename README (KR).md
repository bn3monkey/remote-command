...
- [서버 설치 (미리 빌드된 바이너리)](#서버-설치-미리-빌드된-바이너리)
...
배포 시에는 호스트마다 소스를 빌드하지 않고 미리 빌드된 바이너리를 받는 편이 좋습니다.
태그 릴리스를 푸시하면 GitHub Actions([`.github/workflows/release.yml`](.github/workflows/release.yml))가
플랫폼별 단일 실행 파일을 발행하고, 한 줄짜리 설치 스크립트가 다운로드 → 체크섬 검증 → 설치를 수행합니다.
...
넘깁니다. 둘 중 아무것도 주지 않으면 에러로 중단됩니다.

### Linux / macOS

```bash
# 기본 위치 ($HOME/.local/bin)
...
```powershell
# 플래그가 적용되도록 먼저 내려받은 뒤 위치를 지정해 실행
...
설치 스크립트는 지정한 디렉터리에 바이너리를 `remote-command-server`(Windows는 `.exe`)로
설치하고, SHA-256 체크섬을 검증한 뒤, 제거할 때 쓸 `rm` / `Remove-Item` 명령을 출력합니다.

| 옵션 | 기본값 | 용도 |
|------|--------|------|
| `--path <dir>` / `-Path <dir>` | — | 직접 관리하는 설치 디렉터리(기본 위치 플래그와 동시 사용 불가) |
| `--default_path` / `-DefaultPath` | `~/.local/bin` / `%LOCALAPPDATA%\Programs\remote-command` | 내장 기본 위치 사용 |
| `--repo <owner/repo>` (또는 `REMOTE_COMMAND_REPO`) | `bn3monkey/remote-command` | 다운로드할 GitHub 저장소 |
| `--version <tag>` (또는 `REMOTE_COMMAND_VERSION`) | `latest` | 릴리스 태그(예: `v1.0.0`). 재현 가능한 배포용 |

> 현재는 x86_64 / amd64만 발행하며, 그 외 아키텍처(예: arm64)에서는 설치가 중단됩니다.

### 실행 방법

설치된 바이너리는 소스 빌드와 동일한 인수를 받습니다
(`discovery_port command_port stream_port working_dir`, 모두 선택):

```bash
# 기본값: discovery=9000, command=9001, stream=9002, cwd="."
